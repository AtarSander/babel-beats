...
def generate_transcripts(songs_data, song_name):
    for song_data in songs_data:
        if song_name == song_data["title"]:
            result = transcript(f"""src/main/resources/audio/{song_data["title"]}.mp3""", song_data["language"], " ".join(song_data["lyrics"].split(" ")[:25]))
...
if __name__ == "__main__":
    ...
    generate_transcripts(load_song_data(), "szanty_bitwa")
